...
class Maze(tk.Tk, object):
    ...
    def _build_maze(self):
        ...
        # iot 3
        hell31_center = origin + np.array([UNIT * 15, UNIT * 10])
        self.hell31 = self.canvas.create_rectangle(
            hell31_center[0] - 5, hell31_center[1] - 5,
            hell31_center[0] + 5, hell31_center[1] + 5,
            fill='black')
        hell32_center = origin + np.array([UNIT * 15, UNIT * 11])
        self.hell32 = self.canvas.create_rectangle(
            hell32_center[0] - 5, hell32_center[1] - 5,
            hell32_center[0] + 5, hell32_center[1] + 5,
            fill='black')

        # iot 2
        hell21_center = origin + np.array([UNIT * 5, UNIT * 22])
        self.hell21 = self.canvas.create_rectangle(
            hell21_center[0] - 5, hell21_center[1] - 5,
            hell21_center[0] + 5, hell21_center[1] + 5,
            fill='blue')
        hell22_center = origin + np.array([UNIT * 6, UNIT * 22])
        self.hell22 = self.canvas.create_rectangle(
            hell22_center[0] - 5, hell22_center[1] - 5,
            hell22_center[0] + 5, hell22_center[1] + 5,
            fill='blue')

        # iot 1
        hell11_center = origin + np.array([UNIT * 5, UNIT * 20])
        self.hell11 = self.canvas.create_rectangle(
            hell11_center[0] - 5, hell11_center[1] - 5,
            hell11_center[0] + 5, hell11_center[1] + 5,
            fill='red')
        hell12_center = origin + np.array([UNIT * 5, UNIT * 21])
        self.hell12 = self.canvas.create_rectangle(
            hell12_center[0] - 5, hell12_center[1] - 5,
            hell12_center[0] + 5, hell12_center[1] + 5,
            fill='red')
        hell13_center = origin + np.array([UNIT * 6, UNIT * 21])
        self.hell13 = self.canvas.create_rectangle(
            hell13_center[0] - 5, hell13_center[1] - 5,
            hell13_center[0] + 5, hell13_center[1] + 5,
            fill='red')

        # create oval
        oval_center = origin + np.array([UNIT * 39, UNIT * 39])
    ...
    def step_dq(self, action):
        ...
        elif (next_coords in [self.canvas.coords(self.hell11)]) or (
                next_coords in [self.canvas.coords(self.hell12)]) or (
                next_coords in [self.canvas.coords(self.hell13)]) and self.iot1 == 0:
            reward = 10
            self.iot1 = 1
            done = False
        elif (next_coords in [self.canvas.coords(self.hell21)]) or (
                next_coords in [self.canvas.coords(self.hell22)]) and self.iot2 == 0:
            reward = 10
            self.iot2 = 1
            done = False
        elif (next_coords in [self.canvas.coords(self.hell31)]) or (
                next_coords in [self.canvas.coords(self.hell32)]) and self.iot3 == 0:
            reward = 10
            self.iot3 = 1
            done = False
        else:
